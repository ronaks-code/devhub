...
import { AlertCircle, Check, Copy, FileCode, Sparkles } from "lucide-react";
import { api } from "../../lib/api";
import type { ConfigScope, SkillDef } from "../../lib/types";
import { cn } from "../../lib/utils";
import { Spinner } from "../ui";

/** A scope chip used in the list rows (matches AgentsLibrary/McpManager styling). */
 ...
 * that handler is registered. Both are best-effort and never throw. (Same shape
 * as AgentsLibrary's OpenFile, kept local to avoid coupling the two views.)
 */
  ...
  return (
    <li className="flex flex-col gap-1.5 rounded-xl border border-zinc-800/80 bg-zinc-900/30 px-3.5 py-3">
      <div className="flex items-center gap-2">
        <Sparkles className="h-4 w-4 shrink-0 text-clay-400" />
        ...
        {skill.version ? (
          <span
            ...
            title="Version (from frontmatter)"
          >
            v{skill.version}
          </span>
        ) : null}
      ...
      {skill.description ? (
        <p className="text-[12px] leading-relaxed text-zinc-400">{skill.description}</p>
      ) : (
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <OpenFile filePath={skill.filePath} />
      </div>
    </li>
  );
}
...
/**
 * Read-only library of skills — the global set plus, when a project cwd is
 * available, that project's. Fetched from GET /api/config/skills (engine
 * `listSkills`). Shows each skill's name, description, version, and scope, with
 * copy-path / open-in-VS-Code affordances. No writes: skills are authored as
 * `skills/<dir>/SKILL.md` files; this view surfaces what's installed.
 */
export function SkillsManager({ projectCwd }: { projectCwd?: string }) {
  ...
  if (sorted.length === 0) {
    return (
      <div className="rounded-xl border border-zinc-800/80 bg-zinc-900/30 px-4 py-10 text-center">
        ...
        <div className="text-[13px] font-medium text-zinc-400">No skills found</div>
        <p className="mx-auto mt-1 max-w-sm text-[11.5px] text-zinc-600">
          Add skills under <code className="text-zinc-500">~/.claude/skills/</code>
          {projectCwd ? (
            <>
              {" "}or <code className="text-zinc-500">.claude/skills/</code> in this project
            </>
          ) : null}{" "}
          (each a directory with a <code className="text-zinc-500">SKILL.md</code>) and they'll
          appear here.
        </p>
      </div>
    );
  ...
  return (
    <ul className="flex flex-col gap-2.5">
      {sorted.map((s) => (
        <SkillRow key={`${s.scope}:${s.filePath}`} skill={s} />
      ))}
    </ul>
  );
}
