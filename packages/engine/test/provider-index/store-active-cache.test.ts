...
const realpathObservation = vi.hoisted(() => ({
  db: null as Readonly<{ isTransaction: boolean }> | null,
  inTransactionCalls: 0,
  onInTransaction: null as (() => void) | null,
}));

vi.mock("node:fs", async (importOriginal) => {
  const original = await importOriginal<typeof import("node:fs")>();
  const observedRealpath = ((...args: unknown[]) => {
    if (realpathObservation.db?.isTransaction === true) {
      realpathObservation.inTransactionCalls += 1;
      realpathObservation.onInTransaction?.();
    }
    return Reflect.apply(original.realpathSync, original, args);
  }) as typeof original.realpathSync;
  return {
    ...original,
    realpathSync: observedRealpath,
  };
});
...
function summaryWithCwd(key: NativeTaskKey, cwd: string | null): NativeTaskSummary {
  return Object.freeze({ ...summary(key), cwd });
}
...
function providerIndexSnapshot(
  db: TestDatabase,
): Readonly<Record<(typeof PROVIDER_INDEX_TABLES)[number], readonly unknown[]>> {
  return Object.freeze(Object.fromEntries(PROVIDER_INDEX_TABLES.map((table) => [
    table,
    db.prepare(`SELECT * FROM ${table}`).all(),
  ]))) as Readonly<Record<(typeof PROVIDER_INDEX_TABLES)[number], readonly unknown[]>>;
}

function addPromotedSummary(
  store: ProviderTaskIndexStore,
  provider: "openai" | "anthropic",
  home: string,
  nativeTaskId: string,
): Readonly<{ key: NativeTaskKey; locator: ProviderTaskLocator }> {
  const registration = store.registerHome({ provider, home }, 1);
  const key = Object.freeze({ provider, home, nativeTaskId });
  const locator = Object.freeze({
    version: 1,
    provider,
    homeFingerprint: registration.homeFingerprint,
  ...
  return Object.freeze({ key, locator });
}

afterEach(() => {
  realpathObservation.db = null;
  realpathObservation.inTransactionCalls = 0;
  realpathObservation.onInTransaction = null;
...
describe("ProviderTaskIndexStore active cache surface", () => {
  ...
  it("preserves exact change accounting across multi-turn active snapshot replay and replacement", () => {
    const { store, key, stage } = fixture();
    const task = snapshotWithTurns(key, [["one", "two"], ["three"]]);
    promoteTask(store, stage, key, task, 2, 3);

    expect(store.replaceActiveSnapshot(key, task, 400)).toMatchObject({
      cacheDetail: "snapshot",
      observedAt: 400,
      turns: [{ events: [{ text: "one" }, { text: "two" }] }, { events: [{ text: "three" }] }],
    });
    ...
    const changed = Object.freeze({
      ...
      revision: Object.freeze({
        ...task.revision!,
        fingerprint: "openai:v1:active-cache-changed",
      }),
    });
    expect(store.replaceActiveSnapshot(key, changed, 500)).toMatchObject({
      cacheDetail: "snapshot",
      observedAt: 500,
      turns: [{ events: [{ text: "one" }, { text: "two" }] }, { events: [{ text: "three" }] }],
    });
  ...
  it("preserves an unchanged snapshot without filesystem or child materialization in the writer", () => {
    const { db, store, key, locator, stage } = fixture();
    const task = snapshotWithTurns(key, [["one", "two"], ["three"]]);
    promoteTask(store, stage, key, task, 2, 3);
    const childTables = [
    ...
    const childrenBefore = new Map(childTables.map((table) => [
      table,
      db.prepare(`SELECT * FROM ${table}`).all(),
    ]));
    const originalPrepare = db.prepare.bind(db);
    let inWriterChildMaterializations = 0;
      ...
      value: (sql: string) => {
        ...
        return {
          all: (...parameters: Array<string | number | null>) => {
            if (db.isTransaction) inWriterChildMaterializations += 1;
    ...
    expect(realpathObservation.inTransactionCalls).toBe(0);
    expect(reentrantCalls).toBe(0);
    expect(inWriterChildMaterializations).toBe(0);
    for (const table of childTables) {
      expect(db.prepare(`SELECT * FROM ${table}`).all()).toEqual(childrenBefore.get(table));
    }
  });

  it("keeps failed unchanged-snapshot summary replacement byte-exact without filesystem work", () => {
    const { db, store, key, stage } = fixture();
    const task = snapshotWithTurns(key, [["trusted"]]);
    promoteTask(store, stage, key, task, 1, 1);
    db.exec(`CREATE TRIGGER fail_preserved_summary AFTER UPDATE ON provider_task_cache
      ...
      END`);
    const before = providerIndexSnapshot(db);
    realpathObservation.db = db;
    ...
    expect(realpathObservation.inTransactionCalls).toBe(0);
    expect(providerIndexSnapshot(db)).toEqual(before);
  });

  it.each(["summary demotion", "active generation switch", "sync clear"] as const)(
    "rejects a peer $peerChange between preserved preflight and writer acquisition",
    (peerChange) => {
      const root = tempDirectory();
      const filename = path.join(root, "preserved-preflight-race.db");
      const home = tempDirectory();
      let ownerNow = 100;
      const ownerDb = openDatabase(filename);
      const ownerStore = new ProviderTaskIndexStore(ownerDb, {
        now: () => {
          const value = ownerNow;
          ownerNow += 100;
          return value;
        },
        tokenFactory: () => "preserved-race-owner",
      });
      const registration = ownerStore.registerHome({ provider: "openai", home }, 1);
      const key = Object.freeze({
        provider: "openai",
        home,
        nativeTaskId: "preserved-race-task",
      } as const);
      const locator = Object.freeze({
        version: 1,
        provider: "openai",
      ...
      const initialStage = ownerStore.beginStage({
        provider: locator.provider,
        homeFingerprint: locator.homeFingerprint,
      });
      promoteTask(
        ...
        initialStage,
        key,
        snapshotWithTurns(key, [["trusted"]]),
        1,
        1,
      );
      const peerDb = reopenDatabase(filename);
      let peerNow = 400;
      const peerStore = new ProviderTaskIndexStore(peerDb, {
        now: () => {
          const value = peerNow;
          peerNow += 100;
          return value;
        },
        tokenFactory: () => "preserved-race-peer",
      });
      const originalExec = ownerDb.exec.bind(ownerDb);
      let interposed = false;
      let peerAuthority: ReturnType<typeof providerIndexSnapshot> | null = null;
      Object.defineProperty(ownerDb, "exec", {
        configurable: true,
        value: (sql: string) => {
          if (!interposed && sql === "BEGIN IMMEDIATE") {
            interposed = true;
            if (peerChange === "summary demotion") {
              peerStore.replaceActiveSummary(
                key,
                summary(key, "Peer summary authority"),
                700,
              );
            } else if (peerChange === "active generation switch") {
              const peerStage = peerStore.beginStage({
                provider: locator.provider,
                homeFingerprint: locator.homeFingerprint,
              });
              peerStore.stageSummary(
                peerStage,
                key,
                summary(key, "Peer generation authority"),
              );
              peerStore.promoteStage(peerStage, {
                completedAt: 700,
                providerVersion: "peer-version",
                taskCount: 1,
                turnCount: 0,
                eventCount: 0,
                snapshotCount: 0,
              ...
              peerStore.clearRebuildableCache({
                provider: locator.provider,
                homeFingerprint: locator.homeFingerprint,
              });
            }
            peerAuthority = providerIndexSnapshot(peerDb);
          }
      ...
      expectStoreError(
        () => ownerStore.replaceActiveSummary(key, summary(key), 800),
        "CORRUPT_ROW",
      );

      expect(interposed).toBe(true);
      expect(peerAuthority).not.toBeNull();
      expect(providerIndexSnapshot(peerDb)).toEqual(peerAuthority);
    },
  );
  ...
  it.each([
    ["summary", "active generation", `active_generation = active_generation + 1,
      generation_epoch = generation_epoch + 1`],
    ["summary", "generation epoch", "generation_epoch = generation_epoch + 1"],
    ["summary", "staging authority", `staging_generation = active_generation + 1,
      staging_owner_token = 'hostile-stage', staging_heartbeat_at = 1,
      staging_expires_at = 2, state = 'staging',
      generation_epoch = active_generation + 1`],
    ["snapshot", "active generation", `active_generation = active_generation + 1,
      generation_epoch = generation_epoch + 1`],
    ["snapshot", "generation epoch", "generation_epoch = generation_epoch + 1"],
    ["snapshot", "staging authority", `staging_generation = active_generation + 1,
      staging_owner_token = 'hostile-stage', staging_heartbeat_at = 1,
      staging_expires_at = 2, state = 'staging',
      generation_epoch = active_generation + 1`],
  ] as const)(
    "rolls back active $method replacement when a trigger rewrites $authority",
    (method, _authority, assignment) => {
      const { db, store, key, stage } = fixture();
      promoteSummary(store, stage, key);
      const triggerTarget = method === "summary"
        ? "AFTER UPDATE ON provider_task_cache"
        : "AFTER INSERT ON provider_replay_receipts";
      db.exec(`CREATE TRIGGER rewrite_active_sync ${triggerTarget}
        WHEN NEW.native_task_id = 'task-1'
        BEGIN
          UPDATE provider_sync_state SET ${assignment}
          WHERE provider = NEW.provider
            AND home_fingerprint = NEW.home_fingerprint;
        END`);
      const before = providerIndexSnapshot(db);
  ...
  it.each(["summary", "snapshot"] as const)(
    "rolls back active $method replacement when a trigger rewrites a same-count sibling",
    (method) => {
      const { db, store, key, stage } = fixture();
      promoteSummary(store, stage, key);
      const siblingKey = Object.freeze({ ...key, nativeTaskId: "sibling-task" });
      store.replaceActiveSummary(siblingKey, summary(siblingKey, "Trusted sibling"), 350);
      const triggerTarget = method === "summary"
        ? "AFTER UPDATE ON provider_task_cache"
        : "AFTER INSERT ON provider_replay_receipts";
      db.exec(`CREATE TRIGGER rewrite_active_sibling ${triggerTarget}
        WHEN NEW.native_task_id = 'task-1'
        BEGIN
          UPDATE provider_task_cache SET title = 'Hostile sibling'
          WHERE provider = NEW.provider
            AND home_fingerprint = NEW.home_fingerprint
            AND native_task_id = 'sibling-task'
            AND cache_generation = NEW.cache_generation;
        END`);
      const before = providerIndexSnapshot(db);
  ...
  it.each([
    ["epoch below active", "generation_epoch = active_generation - 1"],
    ["staging generation ahead of epoch", `
      staging_generation = active_generation + 1,
      staging_owner_token = 'list-stage',
      staging_heartbeat_at = 1,
      staging_expires_at = 2,
      state = 'staging',
      generation_epoch = active_generation`],
  ] as const)("fails the entire list page when sync authority has $corruption", (
    _corruption,
    assignment,
  ) => {
    const { db, store, key, stage } = fixture();
    promoteSummary(store, stage, key);
    db.exec(`UPDATE provider_sync_state SET ${assignment}`);

    expectStoreError(() => store.list({}), "CORRUPT_ROW");
  });

  it.each(["sync", "task"] as const)(
    "validates corrupt $authority authority in the limit-plus-one list sentinel",
    (authority) => {
      const { db, store, key, stage } = fixture();
      promoteSummary(store, stage, key);
      store.replaceActiveSummary(key, {
        ...summary(key),
        updatedAt: "2026-07-14T00:02:00.000Z",
      }, 400);
      const sentinel = addPromotedSummary(
        store,
        "openai",
        tempDirectory(),
        "corrupt-sentinel-task",
      );
      if (authority === "sync") {
        db.prepare(`UPDATE provider_sync_state
          SET generation_epoch = active_generation - 1
          WHERE provider = ? AND home_fingerprint = ?`).run(
          sentinel.locator.provider,
          sentinel.locator.homeFingerprint,
        );
      } else {
        db.exec("PRAGMA ignore_check_constraints = ON");
        try {
          db.prepare(`UPDATE provider_task_cache SET status = ''
            WHERE provider = ? AND home_fingerprint = ?`).run(
            sentinel.locator.provider,
            sentinel.locator.homeFingerprint,
          );
      ...
      expectStoreError(() => store.list({ limit: 1 }), "CORRUPT_ROW");
    },
  );

  it("maps missing persisted home authority in global list to CORRUPT_ROW", () => {
    const { db, store, key, locator, stage } = fixture();
    promoteSummary(store, stage, key);
    ...
    expectStoreError(() => store.list({}), "CORRUPT_ROW");
    expectStoreError(() => store.read(locator), "UNKNOWN_HOME");
  });
  ...
  it.each([
    ["summary", "redacted-null"],
    ["summary", "original-null"],
    ["summary", "project-cwd"],
    ["snapshot", "redacted-null"],
    ["snapshot", "original-null"],
    ["snapshot", "project-cwd"],
  ] as const)("decodes and preserves the valid $detail $cwdState CWD state", (
    detail,
    cwdState,
  ) => {
    const { db, store, key, locator, stage } = fixture();
    const projectCwd = cwdState === "project-cwd" ? tempDirectory() : null;
    const inputCwd = cwdState === "redacted-null" ? key.home : projectCwd;
    const expectedCwd = cwdState === "project-cwd" ? projectCwd : null;
    const expectedRedacted = cwdState === "redacted-null";
    const inputSummary = summaryWithCwd(key, inputCwd);
    if (detail === "snapshot") {
      store.stageSnapshot(stage, key, Object.freeze({
        ...inputSummary,
        turns: Object.freeze([]),
      }));
    } else {
      store.stageSummary(stage, key, inputSummary);
    }
    store.promoteStage(stage, {
      completedAt: 300,
      providerVersion: null,
      ...
      turnCount: 0,
      eventCount: 0,
      snapshotCount: detail === "snapshot" ? 1 : 0,
      receiptCount: detail === "snapshot" ? 1 : 0,
    });

    expect(store.list({}).items).toMatchObject([{
      cwd: expectedCwd,
      cwdRedacted: expectedRedacted,
      cacheDetail: detail,
    }]);
    expect(store.read(locator)).toMatchObject({
      cwd: expectedCwd,
    ...
    const childrenBefore = db.prepare(`SELECT * FROM provider_replay_receipts`).all();
    expect(store.replaceActiveSummary(key, inputSummary, 400)).toMatchObject({
      cwd: expectedCwd,
      cwdRedacted: expectedRedacted,
      cacheDetail: detail,
    });
    expect(db.prepare(`SELECT * FROM provider_replay_receipts`).all()).toEqual(childrenBefore);
  });
  ...
  it.each(["durable rows", "foreign cache and sync"] as const)(
    "rolls back invalidation when a trigger mutates $sideEffect",
    (sideEffect) => {
      const { db, store, key, locator, stage } = fixture();
      promoteSummary(store, stage, key);
      ...
      store.requireReconciliation(locator, {
        reviewedFingerprint: null,
        nativeFingerprint: "openai:v1:invalidation-guard",
        writerEpoch: 0,
        reason: "NATIVE_REVISION_MISMATCH",
      });
      const other = addPromotedSummary(
        store,
        "openai",
        tempDirectory(),
        "other-home-task",
      );
      const body = sideEffect === "durable rows"
        ? `DELETE FROM provider_task_meta
            WHERE provider = OLD.provider
              AND home_fingerprint = OLD.home_fingerprint
              AND native_task_id = OLD.native_task_id;
          DELETE FROM provider_reconciliation_state
            WHERE provider = OLD.provider
              AND home_fingerprint = OLD.home_fingerprint
              AND native_task_id = OLD.native_task_id;`
        : `DELETE FROM provider_task_cache
            WHERE provider = '${other.locator.provider}'
              AND home_fingerprint = '${other.locator.homeFingerprint}';
          UPDATE provider_sync_state SET provider_version = 'hostile-rewrite'
            WHERE provider = '${other.locator.provider}'
              AND home_fingerprint = '${other.locator.homeFingerprint}';`;
      db.exec(`CREATE TRIGGER invalidate_side_effect AFTER DELETE ON provider_task_cache
        WHEN OLD.native_task_id = 'task-1'
        BEGIN ${body} END`);
      const before = providerIndexSnapshot(db);
  ...
  it.each(["epoch below active", "epoch below staging"] as const)(
    "rejects clear before mutation when sync authority has $corruption",
    (corruption) => {
      const { db, store, key, locator, stage } = fixture();
      promoteSummary(store, stage, key);
      if (corruption === "epoch below staging") {
        store.beginStage({
          provider: locator.provider,
          homeFingerprint: locator.homeFingerprint,
        });
        db.exec(`UPDATE provider_sync_state
          SET generation_epoch = active_generation`);
      } else {
        db.exec(`UPDATE provider_sync_state
          SET generation_epoch = active_generation - 1`);
      }
  ...
  it.each(["global", "provider"] as const)(
    "clears $scope scope without unbounded sync-row all materialization",
    (scope) => {
      const { db, store, key, locator, stage } = fixture();
      promoteSummary(store, stage, key);
      const originalPrepare = db.prepare.bind(db);
      let forbiddenAllCalls = 0;
      Object.defineProperty(db, "prepare", {
        configurable: true,
        value: (sql: string) => {
          const statement = originalPrepare(sql);
          if (!sql.includes("FROM provider_sync_state")) return statement;
          return {
            all: (..._parameters: Array<string | number | null>) => {
              forbiddenAllCalls += 1;
              throw new Error("unbounded sync materialization");
            },
      ...
      expect(store.clearRebuildableCache(scope === "global"
        ? undefined
        : { provider: "openai", homeFingerprint: null })).toEqual({
        taskCount: 1,
        turnCount: 0,
        eventCount: 0,
        receiptCount: 0,
      ...
      expect(forbiddenAllCalls).toBe(0);
      expect(db.prepare(`SELECT active_generation, staging_generation,
        staging_owner_token, staging_heartbeat_at, staging_expires_at, state,
        provider_version, last_completed_at, generation_epoch
        FROM provider_sync_state
        WHERE provider = ? AND home_fingerprint = ?`).get(
        locator.provider,
        locator.homeFingerprint,
      )).toEqual({
        active_generation: 0,
  ...
  it.each(["global", "provider", "home"] as const)(
    "performs no provider-home canonicalization inside the $scope clear writer",
    (scope) => {
      const { db, store, key, locator, stage } = fixture();
      promoteSummary(store, stage, key);
      realpathObservation.db = db;
      ...
      expect(store.clearRebuildableCache(scope === "global"
        ? undefined
        : scope === "provider"
          ? { provider: locator.provider, homeFingerprint: null }
          : { provider: locator.provider, homeFingerprint: locator.homeFingerprint })).toEqual({
        taskCount: 1,
        turnCount: 0,
        eventCount: 0,
        receiptCount: 0,
  ...
  it.each(["global", "provider", "home"] as const)(
    "rolls back $scope clear when a trigger deletes durable authority",
    (scope) => {
      const { db, store, key, locator, stage } = fixture();
      promoteSummary(store, stage, key);
      db.prepare(`INSERT INTO provider_task_meta (
      ...
      store.requireReconciliation(locator, {
        reviewedFingerprint: null,
        nativeFingerprint: "openai:v1:clear-guard",
        writerEpoch: 0,
        reason: "NATIVE_REVISION_MISMATCH",
      });
      db.exec(`CREATE TRIGGER clear_durable_side_effect
        AFTER DELETE ON provider_task_cache
        WHEN OLD.native_task_id = 'task-1'
        BEGIN
          DELETE FROM provider_task_meta
            WHERE provider = OLD.provider
        ...
        END`);
      const before = providerIndexSnapshot(db);
      const clearScope = scope === "global"
        ? undefined
        : scope === "provider"
          ? { provider: "openai", homeFingerprint: null } as const
          : { provider: "openai", homeFingerprint: locator.homeFingerprint } as const;

      expectStoreError(() => store.clearRebuildableCache(clearScope), "CORRUPT_ROW");

      expect(providerIndexSnapshot(db)).toEqual(before);
    },
  );

  it.each(["provider", "home"] as const)(
    "rolls back $scope clear when a trigger deletes foreign cache and rewrites foreign sync",
    (scope) => {
      const { db, store, key, locator, stage } = fixture();
      promoteSummary(store, stage, key);
      const sameProvider = addPromotedSummary(
        store,
        "openai",
        tempDirectory(),
        "same-provider-foreign-task",
      );
      const otherProvider = addPromotedSummary(
        store,
        "anthropic",
        tempDirectory(),
        "other-provider-foreign-task",
      );
      const foreign = scope === "provider" ? otherProvider : sameProvider;
      db.exec(`CREATE TRIGGER clear_foreign_side_effect
        AFTER DELETE ON provider_task_cache
        WHEN OLD.native_task_id = 'task-1'
        BEGIN
          DELETE FROM provider_task_cache
            WHERE provider = '${foreign.locator.provider}'
              AND home_fingerprint = '${foreign.locator.homeFingerprint}';
          UPDATE provider_sync_state SET provider_version = 'hostile-rewrite'
            WHERE provider = '${foreign.locator.provider}'
              AND home_fingerprint = '${foreign.locator.homeFingerprint}';
        END`);
      const before = providerIndexSnapshot(db);
      const clearScope = scope === "provider"
        ? { provider: "openai", homeFingerprint: null } as const
        : { provider: "openai", homeFingerprint: locator.homeFingerprint } as const;
