...
function registeredHome(
  ...
  if (row === undefined) return fail("UNKNOWN_HOME");
...
function storedNullableBoolean(value: unknown): boolean | null {
  ...
  if (value !== 0 && value !== 1) return fail("CORRUPT_ROW");
  return value === 1;
}
...
function preparedSummaryFromTaskRow(
  ...
  const key = Object.freeze({
    ...
    home: registration.canonicalHome,
  ...
  const preparation = prepareProviderTaskSummaryForStore(
    ...
    Object.freeze({
      ...
      archived: storedNullableBoolean(row.archived),
      source: row.source,
      revision,
    }) as NativeTaskSummary,
  );
  if (preparation.status === "failed") return fail("CORRUPT_ROW");
  const observedAt = safeInteger(row.observed_at);
  if (!taskRowMatchesPrepared(row, preparation.value, generation, observedAt)) {
    return fail("CORRUPT_ROW");
  }
  return Object.freeze({ prepared: preparation.value, observedAt });
}
...
export function listActiveProviderTasks(
  ...
  const items = pageRows.map((row) => {
    ...
    const registration = registeredHome(db, locator.provider, locator.homeFingerprint);
