...
export function registerConfigRoutes(app: FastifyInstance, engine: Engine): void {
  ...
  // Installed plugins (read-only; from `~/.claude/plugins`, not settings.json).
  // Delegates to the engine's published `listPlugins`, the single canonical reader of
  // `installed_plugins.json` (+ blocklist), so each plugin carries its resolved
  // `enabled` flag — the shape the web PluginsView consumes. The engine exposes no
  // marketplace listing, so `marketplaces` is always `[]` here (see integration note);
  // the response object keeps that key so the web `PluginsResult` shape is unchanged.
  // Tolerant of a fresh machine: the engine returns [] when no plugins are installed.
  app.get("/api/config/plugins", async () => ({
    plugins: await engine.listPlugins(),
    marketplaces: [],
  }));
