...
#### Task 3 M5 coordinator observation-lanes exact-tip certification (source `8281ef2`)

- Scope reviewed: the exact lanes diff on `coordinator.ts` (`observeListPage`/`observeTask`, `observationEpoch`, bounded per-locator FIFO lanes, `maxObservationOperations` reservations, per-locator observation epochs) plus its new test file `coordinator-observe.test.ts` (12 tests). The diff is `coordinator.ts` +253 and the new 430-line test only; no other engine, schema, server, web, providers-barrel, or feature-flag surface changed.
- SPEC GO. `observeTask` folds a `turns`-bearing payload through `replaceActiveSnapshot` and a summary-only payload through `replaceActiveSummary` (store preserves matching cached children), returning `null` before initial promotion. `observeListPage` folds each summary, freezes its result array, and admits atomically — an over-budget page fails `CAPACITY` via `reserveObservationOperations` before any store write. Per-locator FIFO lanes chain off `lane.tail` in submission order, are created lazily, are bounded by `maxUniqueTasks`, and are torn down after the last waiter settles with an ABA identity guard. Reservations are taken before the `try` and released in `finally` on every terminal path — success, store failure, `CAPACITY` (both the pre-increment budget check and the `maxUniqueTasks` lane check), and `CLOCK_FAILURE`; `reserveObservationOperations` throws only when it did not increment, so accounting stays balanced. The per-locator epoch bumps only on a mutating (non-null) apply while the lane is live and reads back `0` after teardown. Coordinator owns envelope/lane/reservation/clock validation with value-free codes; payload validation is delegated to the store (`ProviderIndexStoreError`, e.g. `UNKNOWN_HOME`, propagates unchanged). All 12 tests map 1:1 to these clauses.
- QUALITY/SECURITY GO, zero P0-P3 findings. Hostile-input containment matches the established store/coordinator idiom: `utilTypes.isProxy` rejection, null-or-`Object.prototype`-only prototype guard, accessor rejection via `getOwnPropertyDescriptor` + `"value" in descriptor`, and dense-array census via `Reflect.ownKeys`; the "rejects hostile observe input without touching the store" test proves Proxy/null/non-array/extra-key inputs fail before any write. No information leakage: coordinator errors carry fixed value-free messages and a throwing clock maps to `CLOCK_FAILURE` with no path in the message (test asserts no `/tmp/`). The lane is failure-resilient — a rejected observation rebinds `tail` through a rejection-swallowing continuation so subsequent same-locator observations still run, and `result` carries rejection handlers plus is awaited, so there is no unhandled rejection. Resource use is bounded on all three axes (page length by `HARD_LIMITS.maxObservationOperations`, live lanes by `maxUniqueTasks`, reservations by `maxObservationOperations`). No feature-flag default changed; `nativeCodex`/`persistentClaude`/`unifiedTaskIndex` remain false. No repair was required; both reviews returned GO against the un-modified source.
- Exact-tip gate on the promotion tree passes `589/589` provider-index across 15 files, including the new `coordinator-observe.test.ts` (12). `pnpm --filter @devhub/engine typecheck` (`tsc --noEmit`) and the negative public-surface fixture (`tsconfig.public-surface.json`) both pass; staged and full `git diff --check` are clean.
- Only the two reviewed engine files (`coordinator.ts` + `coordinator-observe.test.ts`) were applied to `campaign/auto-improve`; the four user-owned paths (`.gitignore`, `apps/web/src/components/ChatPane.tsx`, `apps/web/src/components/SlashPalette.tsx`, `AGENTS.md`) were never staged. This is the Task 3 observation-lanes promotion checkpoint; overall milestone progress remains `3/9 = 33%`.
