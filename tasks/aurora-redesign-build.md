...
- [~] Chat surface glass: composer/inspector/canvas glass + aurora done via CSS (commit 83b0c77).
  ...
  - [ ] STRUCTURAL-C (HANDED OFF — the risky gut, do as its own reviewed change): InspectorDock repurpose from the 5-destination Diff/Files/Terminal/Browser/Artifacts tablist → fixed WORKTREE + SESSION + CHANGED-FILES sections (§3.3 inspector). Blast radius: rewrite `components/features/inspectors/InspectorDock.tsx` (new props: worktree/session/changedFiles instead of selected/onSelectDestination/content) + BOTH call sites (`ChatHost.tsx:187` and Browse path `App.tsx:2081`) + m6 builders (`buildEnvironmentSummary` keep→worktree; replace `buildDiffContent`/`buildFilesContent` with a `buildChangedFiles(fileChanges)→{path,added,removed}[]`) + rewrite the 578-line `InspectorDock.test.ts` (all tablist/roving/destination assertions). MUST preserve exports still imported elsewhere: `nextTabIndex` (used by `components/features/settings/settings-ui.tsx`), `resolveInspectorDockMode`/`isInspectorDockApplied`, the `disclosure` variant, `INSPECTOR_GEOMETRY`. Data honesty: no worktree → "⎇ main · no worktree" quiet state; a row renders only when backed. The Environment region already surfaces branch+changes honestly today (not diff-forward-broken), so this is a redesign-layout change, not a bug fix — safe to defer + review.
