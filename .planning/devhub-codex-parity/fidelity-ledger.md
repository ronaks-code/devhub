...
| Provider setup | `browser-wide-create-claude.png`, `browser-narrow-create-claude.png` vs concept 5 | Fixed provider, disabled model selection, Claude permission mode, cwd, and first message are present. Older screenshots predate the exact-copy repair; final recapture remains required. |
| Permission vocabulary | Source/test evidence in `ClaudeNativePane.test.ts` | Labels are now exact safe installed terms `Manual`, `Accept edits`, `Plan`; no Codex `Workspace` appears. Match after test-first repair. |
| Model disclosure | Source/test evidence | Exact allowed capability copy replaces an overlong implementation sentence. Match after test-first repair; divergence detail remains available from runtime diagnostics, not a fake picker. |
| Provider-lock disclosure | Source/test evidence | Exact approved sentence now states that continuation elsewhere requires a fork. Match after test-first repair. |
...
| Responsive geometry | Wide/narrow captures | `1280` and `768` document widths have no horizontal overflow. Functional pass; the extra permanent rails remain an intentional pre-M6 mismatch. |
| Computer Use | `computer-use-chrome-devhub.jpg`, `computer-use.md` | Chrome/DevHub workflow and accessibility state pass; first-party Codex control is host-blocked and is not relabeled. Honest partial. |
...
The existing-UI Anthropic slice is functionally and semantically coherent under deterministic fixtures, with exact-copy and task-status defects repaired test-first. Final screenshot recapture and independent review remain before deterministic closeout. The raw selected-runtime lifecycle artifact is still missing, so the feature flag remains false and M4 is not complete.
