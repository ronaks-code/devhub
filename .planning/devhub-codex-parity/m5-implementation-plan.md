...
9. The v14 base migration and additive v15 generation-epoch repair are atomic and idempotent. Rollback means an older build can ignore the new tables and continue using legacy routes/settings; it does not mean destructive down-migration.
...
## Task 2: Additive v14 base, v15 generation epoch, and store primitives
...
- Every cache/turn/event/receipt primary and foreign key includes `cache_generation`. Sync state permits only one token-owned staging generation with a bounded heartbeat/expiry and promotes it to active in one transaction after a complete census. Reads join only `provider_sync_state.active_generation`; an interrupted staging generation is never visible. The additive v15 `generation_epoch` is a safe nonnegative allocation watermark: it is always at least `active_generation`, and a staging row requires `staging_generation = generation_epoch > active_generation`; idle rows may retain an epoch above active so aborted identities are never reusable.
...
- Historical v14 remains byte-for-byte pinned. The appended v15 migration adds only `generation_epoch`, backfills it to `max(active_generation, coalesce(staging_generation, active_generation))`, and validates the exact latest shape and cross-column invariants before commit. Fresh and v13 upgrade paths converge on the same v15 shape. Claimed-v15 damage fails without healing; v14-to-v15 reruns are idempotent; migration runs in the existing per-version transaction and never rewrites legacy or provider-index payload rows.
...
- Reconciliation `require` is an internal adapter/store operation, never a browser primitive: its locator comes from a validated provider-native key or an already persisted durable identity. The authenticated acknowledgement route must perform its authoritative native/deletion reread before the exact store CAS. Once a home is removed, its former raw path is intentionally unavailable and must not be reconstructed or retained merely to re-run containment; orphan operations continue to enforce the canonical locator grammar, fixed 64-hex home fingerprint, Unicode/size bounds, and secret-redaction fixed points. When the home is still registered, raw-home exclusion remains mandatory.
...
- `stageSummary` is last-call-wins for the exact staged task. If its snapshot-hashed summary/revision fields differ from an existing staged snapshot, it deletes that exact staged task row (cascading only its staged turns/events/receipt) and inserts a summary-only row. If those fields are unchanged, it preserves the complete snapshot children and receipt.
- A replay-key conflict samples the injected clock once before its owned transaction, commits the stage abort and one reconciliation latch with `reviewedFingerprint:"provider-index-snapshot:v1:<existingSnapshotFingerprint>"`, `nativeFingerprint:"provider-index-snapshot:v1:<incomingSnapshotFingerprint>"`, `writerEpoch:0`, and `reason:"REPLAY_CONFLICT"`, then throws `REPLAY_CONFLICT` only after commit.
- `beginStage` is an atomic recovery boundary. With no stage it allocates `generation_epoch + 1` and a random owner token. With an unexpired foreign stage it returns `STAGE_BUSY`. With an expired non-active stage it deletes only that abandoned generation and allocates the next epoch in the same transaction. Epoch overflow fails `CAPACITY` without mutation. Every stage write/heartbeat verifies token+generation and monotonically extends the bounded expiry; a lost/expired token aborts the coordinator.
- `promoteStage` verifies the unexpired stage token, scope, exact completion counts, provider version, and row bounds, then changes `active_generation` in one transaction before retiring older generations while preserving `generation_epoch`. `abortStage`, replay-conflict cleanup, and cache clearing delete only their exact cache/staging state and preserve the epoch watermark. `list`/`read` never query staging rows.
...
- `replaceActiveSummary` has the same active-generation prerequisite. If its snapshot-hashed summary/revision fields are unchanged, it updates the summary observation while preserving every existing turn/event/receipt child. If those fields changed, it deletes the exact active task row (cascading only its cache children) and inserts a summary-only row so no stale snapshot can remain attached to changed summary authority. It returns `null` before the first complete promotion.
